...
const MilestoneProgress = ({ booking }: { booking: SurveyBooking }) => {
  const current = resolveSurveyMilestone(booking.current_milestone, booking.status);
  const lifecycle = resolveSurveyLifecycle(booking.journey_status, booking.current_milestone, booking.status);
  const position = surveyMilestonePosition(current);
  ...
  return (
    <div className="min-w-44">
      <div className="mb-1.5 flex items-center justify-between gap-2 text-xs">
        ...
        <span className="text-slate-500">{completed} of 5</span>
      </div>
      <div className="h-2 overflow-hidden rounded-full bg-slate-100">
        <div className={`h-full rounded-full ${lifecycle === "active" ? "bg-amber-400" : "bg-slate-400"}`} style={{ width: `${(completed / 5) * 100}%` }} />
      </div>
        ...
        <div className="space-y-5 p-5">
          ...
          <section className="rounded-xl border border-slate-200 p-4">
            ...
            {history.length ? <div className="space-y-3">{history.map((entry) => <div key={entry.id} className="flex gap-3 border-l-2 border-amber-300 pl-3 text-sm">
              <div className="min-w-0"><div className="font-bold">{surveyMilestoneMeta[resolveSurveyMilestoneState(entry.new_milestone)].label}</div><div className="text-xs text-slate-500">{formatDateTime(entry.created_at)}{entry.updated_by ? ` · ${entry.updated_by}` : ""}</div>{entry.note ? <div className="mt-1 text-slate-600">{entry.note}</div> : null}</div>
...
const ProgressModal = ({
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      setError("");
      onMutationStart();
      if (milestone === "survey_scheduled" && !confirmedAt) throw new Error("Please provide the confirmed survey date and time.");
      const movingBackward = milestone !== "cancelled" && milestone !== "on_hold"
        && surveyMilestonePosition(milestone) < surveyMilestonePosition(current);
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" onMouseDown={onClose}>
    <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="space-y-5 p-5">
        <div><div className="mb-3 text-xs font-black uppercase tracking-wide text-slate-500">Milestone stepper</div><div className="grid gap-2 sm:grid-cols-3">{surveyMilestones.map((item, index) => {
          const selected = item === milestone;
          const achieved = index <= surveyMilestonePosition(current);
        ...
        {milestone === "survey_scheduled" ? <label className="block text-sm font-bold text-slate-700">Confirmed survey date and time<input className="field mt-1" type="datetime-local" value={confirmedAt} onChange={(event) => setConfirmedAt(event.target.value)} /></label> : null}
...
export function BookingsPage() {
  ...
  const rows = useMemo(() => (query.data ?? []).filter((booking) => {
    ...
    return filter === "on_hold" || filter === "cancelled"
      ? lifecycle === filter
      : resolveSurveyMilestone(booking.current_milestone, booking.status) === filter;
  }), [filter, query.data]);
  ...
  return <>
    ...
    <div className="panel mb-4 flex flex-wrap items-center justify-between gap-3 p-3"><select className="field max-w-64" value={filter} onChange={(event) => setFilter(event.target.value as typeof filter)}><option value="all">All milestones</option>{surveyMilestones.map((item) => <option key={item} value={item}>{surveyMilestoneMeta[item].label}</option>)}<option value="on_hold">On Hold</option><option value="cancelled">Cancelled</option></select><div className="text-xs font-semibold text-slate-500">{rows.length} solar survey booking{rows.length === 1 ? "" : "s"}</div></div>
